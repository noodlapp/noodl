export const node = {
  name: 'noodl.playground.response',
  displayNodeName: 'Response',
  category: 'Cloud',
  docs: 'https://docs.noodl.net/nodes/playground-functions/response',
  useVariants: false,
  mountedInput: false,
...
export function setup(context, graphModel) {
  ...
  graphModel.on('editorImportComplete', () => {
    graphModel.on('nodeAdded.noodl.playground.response', function (node) {
      _managePortsForNode(node);
    });

    for (const node of graphModel.getNodesWithType('noodl.playground.response')) {
      _managePortsForNode(node);
    }
  });
}
