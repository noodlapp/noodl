...
export const node = {
  name: 'noodl.playground.request',
  displayNodeName: 'Request',
  category: 'Cloud',
  docs: 'https://docs.noodl.net/nodes/playground-functions/request',
  useVariants: false,
  mountedInput: false,
...
export function setup(context, graphModel) {
  ...
  graphModel.on('editorImportComplete', () => {
    graphModel.on('nodeAdded.noodl.playground.request', function (node) {
      _managePortsForNode(node);
    });

    for (const node of graphModel.getNodesWithType('noodl.playground.request')) {
      _managePortsForNode(node);
    }
  });
}
